...
process diamond {
    ...
    diamond diamondp --ultra-sensitive -p ${task.cpus} -q ${fasta} -d ${fasta2}.dmnd -e ${params.evalue} --outfmt 6 qseqid sseqid pident length mismatch gapopen qstart qend qlen sstart send evalue bitscore slen | awk '{if(\$3>${params.seqidentity*100} && \$4>(\$9*${params.alnlength})){print \$0}}' > ${name}-query-${name2}-db.diamond
...
/* Comments:
When blast was used:
I removed the -parse_seqids parameter from the makediamonddb command because of an error with fasta IDs that are longer than 50 chars. strange.
*/