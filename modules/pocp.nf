/*
Calculate POCP value:
  pocp = (((c1.to_f + c2)/(t1 + t2)) * 100).round(4)
  puts "\n\nC1:\t#{c1}\nC2:\t#{c2}\n\nT1:\t#{t1}\nT2:\t#{t2}\n\nPOCP = [(C1+C2)/(T1+T2)]*100% = #{pocp}"
*/
process pocp {
    label 'pocp'
    publishDir "${params.output}/pocp", mode: 'copy', pattern: "*.txt"

    input:
      tuple val(genome_names), path(proteins), path(blast_hits)

    output:
	    tuple val(genome_names), path("${genome_names}.txt"), emit: pocp

    script:
    """
    t1=\$(grep -c "^>" ${proteins[0]} || true)
    t2=\$(grep -c "^>" ${proteins[1]} || true)
    c1=\$(cat ${blast_hits[0]})
    c2=\$(cat ${blast_hits[1]})

    if [ \$(( t1 + t2 )) -eq 0 ]; then
      echo "ERROR: no protein sequences found in ${proteins[0]} and ${proteins[1]}" >&2
      exit 1
    fi

    POCP=\$(python -c "print((((\$c1 + \$c2) / (\$t1 + \$t2)) * 100))")
    echo \$POCP > ${genome_names}.txt
    """
}

process pocp_matrix {
    label 'ruby'
    publishDir "${params.output}", mode: 'copy', pattern: "pocp-matrix.tsv"

    input:
      path(pocp_files)

    output:
	    path('pocp-matrix.tsv')

    script:
    """
    pocp-matrix.rb
    """

}
